...
$collegeid = optional_param('collegeid', $requests['collegeid'], PARAM_RAW);
$departmentid = optional_param('departmentid', $requests['departmentid'], PARAM_RAW);
require_login();
    ...
    case 'departmentcourses':
        if ($collegeid > 0) {
            $sql = "SELECT DISTINCT c.id, c.fullname 
                    FROM mdl_course c 
                    WHERE (c.open_parentcourseid != 0 OR (c.open_parentcourseid = 0 AND c.forpurchaseindividually = 2 )) AND c.open_costcenterid = $collegeid";
            if ($departmentid > 0) {
