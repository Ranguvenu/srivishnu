...
if (is_siteadmin($USER->id) || (new ls)->is_manager($USER->id, $_SESSION['ls_contextlevel'], $_SESSION['role'])) {
    $dashboarddepartmentid = $DB->get_field_sql("SELECT id FROM {local_costcenter} WHERE id <> 0 AND parentid = $dashboardcollegeid ORDER BY id ASC LIMIT 0, 1");
    $dashboarddepartmentid = 0;
} else {
    $dashboarddepartment = (new querylib)->get_roledepartments($USER->id, $role, $dashboardcollegeid, 0, '', ' LIMIT 0, 1', false, false, '', true);
    if ((!is_siteadmin() && !has_capability('local/costcenter:manage_multiorganizations', $systemcontext)) && has_capability('local/costcenter:manage_ownorganization',$systemcontext) && has_capability('block/learnerscript:managelsreports',$systemcontext)) {
        ...
        if (!empty($dashboarddepartment)) {
            $dashboarddepartmentid = array_keys($dashboarddepartment)[0];
        }
    }
}
...
if (!empty($role) || is_siteadmin()) {
    ...
    if($configuredinstances > 0){
        echo "<input type='hidden' name='filter_college' id='ls_collegeid' class = 'report_college'  value='" . $dashboardcollegeid . "' />";
        echo "<input type='hidden' name='filter_department' id='ls_departmentid' class = 'report_departments'  value='" . $dashboarddepartmentid . "' />";
