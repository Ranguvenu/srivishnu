...
class report_inactivecourses extends reportbase implements report {
    /**
    ...
    function select() {
          $this->sql = "SELECT DISTINCT concat(c.id,'-',u.id), c.fullname AS course, lc.fullname AS college, lco.fullname AS department, lp.fullname AS program, concat(u.firstname,' ',u.lastname) AS faculty";
        parent::select();
    }
    ...
    public function joins() {
        $this->sql .= " JOIN {course} c ON c.id=lsl.courseid AND c.visible=1 
                        JOIN {user} u ON u.id=lsl.userid AND u.deleted=0 AND u.confirmed=1 
                        JOIN {role_assignments} ra ON ra.userid=u.id 
                        JOIN {role} r ON r.id=ra.roleid AND r.shortname='faculty'
                        LEFT JOIN {course} pc ON pc.id=c.id AND pc.open_parentcourseid!=0 
                        ...
                        LEFT JOIN {course} mc ON mc.id=c.id AND mc.open_parentcourseid=0 AND mc.forpurchaseindividually=2
                        LEFT JOIN {local_costcenter} lco ON lco.id = c.open_departmentid AND lco.parentid = lc.id AND lco.univ_dept_status = 0";
    ...
    function where() {
        ...
        $this->sql .= " WHERE lsl.action='created' AND lsl.target = 'course_module' 
                        AND FROM_UNIXTIME(lsl.timecreated, '%Y-%m-%d') < (curdate() - interval 90 day) ";
