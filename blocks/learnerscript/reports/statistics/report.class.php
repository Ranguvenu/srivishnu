...
class report_statistics extends reportbase {
	...
	function prepare_sql($sql) {
		...
		if ($this->config->name == 'Inactive Courses') {
			$linkreportid = $DB->get_field_sql("SELECT id FROM {block_learnerscript} WHERE type = 'inactivecourses'", array(), IGNORE_MULTIPLE);
		} else if ($this->config->name == 'Active Courses') {
			$linkreportid = $DB->get_field_sql("SELECT id FROM {block_learnerscript} WHERE type = 'coursefiles'", array(), IGNORE_MULTIPLE);
		} else if ($this->config->name == 'Gradable Activity Courses') {
			$linkreportid = $DB->get_field_sql("SELECT id FROM {block_learnerscript} WHERE type = 'coursewithgradable'", array(), IGNORE_MULTIPLE);
		} else if ($this->config->name == 'InActive Faculty') {
			$linkreportid = $DB->get_field_sql("SELECT id FROM {block_learnerscript} WHERE type = 'inactivefaculty'", array(), IGNORE_MULTIPLE);
		} else if ($this->config->name == 'Active Faculty') {
			$linkreportid = $DB->get_field_sql("SELECT id FROM {block_learnerscript} WHERE type = 'activefaculty'", array(), IGNORE_MULTIPLE);
		}
		$sql = str_replace('%%REPORTID%%', $linkreportid, $sql);
		if ((!is_siteadmin() && !has_capability('local/costcenter:manage_multiorganizations', $systemcontext)) && has_capability('local/costcenter:manage_ownorganization',$systemcontext) && has_capability('block/learnerscript:managelsreports',$systemcontext)) {
            $rolewise = " AND u.open_costcenterid = $USER->open_costcenterid AND c.open_costcenterid = $USER->open_costcenterid ";
            ...
            $rolewise = " AND u.open_departmentid = $USER->open_departmentid  AND c.open_departmentid = $USER->open_departmentid ";
        ...
        if (preg_match("/%%FILTER_COLLEGE:([^%]+)%%/i", $sql, $output) && $this->collegeid > 0) {
			$replace = ' AND ' . $output[1] . ' = ' . $this->collegeid;
			$sql = str_replace('%%FILTER_COLLEGE:' . $output[1] . '%%', $replace, $sql);
		}
		...
		if (preg_match("/%%FILTER_PROGRAM:([^%]+)%%/i", $sql, $output) && $this->programid > 0) {
			$replace = ' AND ' . $output[1] . ' = ' . $this->programid;
			$sql = str_replace('%%FILTER_PROGRAM:' . $output[1] . '%%', $replace, $sql);
		}
		if (preg_match("/%%FILTER_FACULTY:([^%]+)%%/i", $sql, $output) && $this->facultyid > 0) {
			$replace = ' AND ' . $output[1] . ' = ' . $this->facultyid;
			$sql = str_replace('%%FILTER_FACULTY:' . $output[1] . '%%', $replace, $sql);
		}

		if ($this->collegeid > 0) {
			$sql = str_replace('%%COLLEGEID%%', $this->collegeid, $sql);
		}
		if ($this->departmentid > 0) {
			$sql = str_replace('%%DEPARTMENTID%%', $this->departmentid, $sql);
		}
		
        $sql = str_replace('%%ROLEWISE%%', $rolewise, $sql);
