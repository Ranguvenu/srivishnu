...
class block_learnerscript_external extends external_api {
    ...
    public static function generate_plotgraph_parameters() {
        return new external_function_parameters(
            array(
                ...
                'collegeid' => new external_value(PARAM_INT, 'College ID', false),
                'departmentid' => new external_value(PARAM_INT, 'Department ID', false)
            )
        );
    }
    public static function generate_plotgraph($reportid, $courseid, $cmid, $status, $userid,
        ...
        $container, $filters, $basicparams, $columnDefs, $reportdashboard, $collegeid, $departmentid) {
        global $DB, $PAGE, $CFG;
        ...
        $reportclass->collegeid = (isset($collegeid) && $collegeid != 0) ? $collegeid : ($reportclass->params['filter_college'] ? $reportclass->params['filter_college'] : 0);
        $reportclass->departmentid = (isset($departmentid) && $departmentid != 0) ? $departmentid : ($reportclass->params['filter_department'] ? $reportclass->params['filter_department'] : 0);
            ...
            if ($report->type != 'statistics') {
                ...
                if ($reporttype == 'pie') {
                    ...
                    if (!empty($reportclass->finalreport->table->data[0])) {
                        foreach ($reportclass->finalreport->table->data[0] as $k => $r) {
                            if ($report->name == 'Inactive Courses' || $report->name == 'Active Courses' || $report->name == 'Gradable Activity Courses' || $report->name == 'Active Faculty' || $report->name == 'InActive Faculty') {
                                $r = '<h1>'.$r.'</h1>';
                            } else {
                                $r = strip_tags($r);
                                $r = is_numeric($r) ? $r : $r;
