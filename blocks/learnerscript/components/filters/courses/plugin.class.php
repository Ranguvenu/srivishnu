...
class plugin_courses extends pluginbase {
    ...
    public function filter_data($selectoption = true, $request = array()){
        ...
        if ($this->reportclass->basicparams) {
            ...
            if (in_array('college', $basicparams) && $basicparams[0] == 'college') {
                $collegeoptions = (new \block_learnerscript\local\querylib)->filter_get_colleges($this, false);
                $collegeids = array_keys($collegeoptions);
                if (empty($request['filter_college'])) {
                    $coursecollegeid = array_shift($collegeids);
                } else {
                    $coursecollegeid = $request['filter_college'];
                }
            } else {
                $coursecollegeid = 0;
            }
            if (in_array('department', $basicparams) && $basicparams[1] == 'department') {
                $departmentoptions = (new \block_learnerscript\local\querylib)->filter_get_departments($this, false, $coursecollegeid);
                $departmentids = array_keys($departmentoptions);
                if (empty($request['filter_department'])) {
                    $coursedepartmentid = array_shift($departmentids);
                } else {
                    $coursedepartmentid = $request['filter_department'];
                }
            } else {
        ...
        $courseoptions = (new \block_learnerscript\local\querylib)->filter_get_courses($this, $selectoption, false, $filtercourse, $filterdata, false, $courseuserid, $filtercourses, $coursecollegeid, $coursedepartmentid);  
