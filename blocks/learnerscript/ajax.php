...
$collegeid = optional_param('collegeid', $collegeid, PARAM_INT);
$departmentid = optional_param('departmentid', $departmentid, PARAM_INT);
...
case 'categorycourses':
	...
	break;
case 'coursefaculties':
	if ($courseid > 0) {
		$faculties = $DB->get_records_sql_menu("SELECT u.id, concat(u.firstname,'',u.lastname) AS fullname FROM {user} u
			JOIN {user_enrolments} ue ON ue.userid = u.id
            JOIN {enrol} e ON e.id = ue.enrolid 
            JOIN {role_assignments} ra ON ra.userid = ue.userid
            JOIN {context} ct ON ct.id = ra.contextid
            JOIN {role} rl ON rl.id = ra.roleid AND rl.shortname = 'faculty' 
            JOIN {course} c ON c.id = e.courseid WHERE c.id = :courseid AND c.visible = :visible", ['courseid' => $courseid, 'visible' => 1]);
		$return = array(0 => 'Select Faculty') + $faculties;
	} else {
		$return = array('' => 'Select Faculty');
	}
	break;
case 'designdata':
	$return = array();
 	...
 	$systemcontext = context_system::instance();
		if ($collegeid > 0) {
			if (is_siteadmin() || ($_SESSION['role'] == 'manager')) {
				$departments = $DB->get_records_sql_menu("SELECT * FROM {local_costcenter} WHERE parentid = $collegeid AND univ_dept_status = 0");
                    ...
                    $departments = $DB->get_records_sql_menu("SELECT * FROM {local_costcenter} WHERE parentid = $USER->open_costcenterid ");
                    ...
                    $departments = $DB->get_records_sql_menu("SELECT * FROM {local_costcenter} WHERE parentid = $USER->open_costcenterid AND univ_dept_status =0 ");
            }else {
                   $departments = $DB->get_records_sql_menu("SELECT * FROM {local_costcenter} WHERE 1 = 1 AND parentid = $collegeid AND id = $USER->open_departmentid");
                }
			$return = array(0 => 'Select Department') + $departments;
		} else {
			$return = array('' => 'Select Department');
		}
		break;
	case 'coldeptprograms':
		if ($collegeid > 0) {
			$sql = "SELECT * FROM {local_program} WHERE costcenter = $collegeid";
