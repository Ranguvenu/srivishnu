...
$learnerscriptnode = '';
if(is_siteadmin() || has_capability('local/program:manageprogram', context_system::instance()) || has_capability('block/learnerscript:managelsreports',context_system::instance()) || has_capability('block/learnerscript:manageownreports', context_system::instance())) {
    $managemoochcourse_string=get_string('lsreports','block_learnerscript');
}
if(is_siteadmin()) {
    $learnerscriptnode .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_learnerscript', 'class'=>'pull-left user_nav_div users dropdown-item'));
    $dashboardurl = 'VishnuLearning';
        $learnerscript_url = new moodle_url('/blocks/reportdashboard/dashboard.php?dashboardurl='.$dashboardurl.'');
          
        $users = html_writer::link($learnerscript_url, '<i class="fa fa-columns" aria-hidden="true"></i><span class="user_navigation_link_text">'.$managemoochcourse_string.'</span>',array('class'=>'user_navigation_link'));
    $learnerscriptnode .= $users;
    $learnerscriptnode .= html_writer::end_tag('li');
}
 if (!is_siteadmin() && has_capability('block/learnerscript:manageownreports',context_system::instance())) {
    $managemoochcourse_string=get_string('lsreports','block_learnerscript');
     $dashboardurl = 'VishnuLearning';
     $userrolesql = "SELECT $limit CONCAT(ra.roleid, '_',c.contextlevel) AS rolecontext, r.shortname, c.contextlevel
            ...
            $dashboardurl = 'VishnuLearning';
       $learnerscript_url = !empty($userroles) ? new moodle_url('/blocks/reportdashboard/dashboard.php?role='.$roleshortname.'&contextlevel='.$rolecontextlevel.'&dashboardurl='.$dashboardurl.'')  : new moodle_url('/blocks/reportdashboard/dashboard.php?dashboardurl='.$dashboardurl.'');
       $users = html_writer::link($learnerscript_url, '<i class="fa fa-columns" aria-hidden="true"></i><span class="user_navigation_link_text">'.$managemoochcourse_string.'</span>',array('class'=>'user_navigation_link'));
    $learnerscriptnode .= $users;
    $learnerscriptnode .= html_writer::end_tag('li');
}
return $learnerscriptnode;
}
