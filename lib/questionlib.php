...
//         $org= $DB->get_field('local_costcenter','shortname',array('id'=>$USER->open_costcenterid));
//         $quizcategory = $DB->get_field("question_categories",'id',array('name'=>$org));



//         if($quizcategory)
//             $sql .= " AND (c.id=$quizcategory OR c.parent=$quizcategory)";
//         else
//             return false;
//         $sql .= " $topwhere AND c.contextid IN (SELECT mc.id FROM {context} AS mc
//             JOIN {course_categories} AS cc ON cc.id = mc.instanceid AND mc.contextlevel = 40
//             JOIN {local_costcenter} AS lc ON lc.category = cc.id
//             WHERE lc.id = {$USER->open_costcenterid}) ";
//         $sql .= " AND c.contextid != 1 ";

//     // }else{
//     }
//         $sql .= " AND c.contextid IN ($contexts) ";
