...
require($CFG->dirroot.'/local/groups/edit_form.php');
...
if ($show && $groups->id && confirm_sesskey()) {
    if (!$groups->visible) {
        ...
        $localgroups = $DB->get_record('local_groups', array('cohortid'=>$groups->id), '*', MUST_EXIST);
        $record->costcenterid = $localgroups->costcenterid;
        $record->departmentid = $localgroups->departmentid;
