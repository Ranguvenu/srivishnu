...
class local_groups_external extends external_api {
     ...
     */
    public static function submit_groupsform_data_parameters() {
     ...
     */
    public function submit_groupsform_data($contextid, $jsonformdata){
        global $PAGE, $CFG;

        require_once($CFG->dirroot . '/local/groups/lib.php');
        // We always must pass webservice params through validate_parameters.
        $params = self::validate_parameters(self::submit_groupsform_data_parameters(),
        ...
        $warnings = array();
        $mform = new group_edit_form(null, array(), 'post', '', null, true, $data);
        
        $valdata = $mform->get_data();
        if($valdata){
            if($valdata->id>0){
               // $updateclass = new local_groups\events\update();
                $success = local_groups_update_groups($valdata);
            } else{
              //  $insertclass = new local_groups\events\insert();
              //  print_object($valdata);
                $success = local_groups_add_groups($valdata);
            }
        } else {
            // Generate a warning.
    ...
    public static function submit_groupsform_data_returns() {
