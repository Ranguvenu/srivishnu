...
$costcenter = optional_param('costcenter','',PARAM_INT);
$depart = optional_param('dept','',PARAM_INT);
...
$size=optional_param('size','', PARAM_RAW);
...
$PAGE->set_url('/local/groups/assign.php', array('id'=>$id, 'costcenter' => $costcenter));
$PAGE->set_pagelayout('admin');
$url = new moodle_url('/local/groups/assign.php', array('id' => $id,'dept' => $depart,'costcenter' => $costcenter));
...
$PAGE->navbar->add(get_string('assigncohorts', 'local_groups'));

$PAGE->set_title(get_string('assigncohorts', 'local_groups'));

$PAGE->set_heading(get_string('assigncohorts', 'local_groups'));
...
if(!empty($size)){
        ...
        $table->head  = array(get_string('firstname', 'local_groups'),get_string('lastname', 'local_groups'),get_string('group', 'local_groups')); 

        $table->align = array('center','center','center');
        $table->size = array('33','33','40');
        $departmentid = $DB->get_field('local_groups','departmentid',array('cohortid' => $id));
        $users = $DB->get_records_sql("SELECT u.* FROM {user} u JOIN {cohort_members} cm ON cm.userid = u.id JOIN {local_groups} lg ON lg.cohortid = cm.cohortid WHERE cm.cohortid = ".$id." AND u.open_departmentid =".$departmentid);
        $data = array();
        foreach($users as $user){
          $row = array();
          $row[] = $user->firstname;
          $row[] = $user->lastname;
          $row[] = $DB->get_field('cohort','name',array('id'=>$id));
          $data[] = $row;
        }    
        if (!empty($users)) {
            ...
            $string .= html_writer::table($table);
            $string .= '<span class="msgdisplay">No Enrolled users available in this table</span>';
            echo $string;
        }
...
if ($groups) {
 ...
 // $mform = new filters_form($url, array('filterlist'=>$filterlist,'enrolid'=>0, 'courseid'=>$id));

  //Revathi Issue ODL-803 starts
  $mform = new filters_form($url, array('filterlist'=>$filterlist,'enrolid'=>0, 'courseid'=>$id,'action' => 'user_enrolment'));
  //Revathi Issue ODL-803 ends
  ...
  if ($mform->is_cancelled()) {
    ...
    $options = array('context' => $context->id, 'groupsid' => $id, 'organization' => $organization,'dept' => $depart,  'department' => $department, 'email' => $email, 'idnumber' => $idnumber, 'uname' => $uname);
  
    if ( $add AND confirm_sesskey()) {        
        if($submit_value == "Add_All_Users"){
			$options =json_decode($_REQUEST["options"],false);
              $userstoassign=array_flip(local_group_users('add', $id, (array)$options, false, $offset1=-1, $perpage=-1,$lastitem=0,$costcenter,$depart));
        }else{
            $userstoassign =$add;
        }
    ...
    if ( $remove AND confirm_sesskey()) {        
        if($submit_value=="Remove_All_Users"){
			$options =json_decode($_REQUEST["options"],false);
             $userstounassign = array_flip(local_group_users('remove',$id,(array)$options,false,$offset1=-1,$perpage=-1,$lastitem=0,$costcenter,$depart));
        }else{
    ...
    $select_to_users = local_group_users('add', $id, $options, false, $offset=-1, $perpage=50,'', $costcenter,$depart);
    $select_to_users_total = local_group_users('add', $id, $options, true, $offset1=-1, $perpage=-1,' ',$costcenter,$depart);
 
    $select_from_users = local_group_users('remove', $id, $options, false, $offset1=-1, $perpage=50,'', $costcenter,$depart);
    $select_from_users_total = local_group_users('remove', $id, $options, true, $offset1=-1, $perpage=-1,' ',$costcenter,$depart);
   ...
   <label>'.get_string('enrolled_users', 'local_groups',$select_from_users_total).$select_all_not_enrolled_users.'</label>';
   ...
   $content.='<form  method="post" name="form_name" id="user_un_assign" class="form_class" ><button type="submit" class="custom_btn btn move btn-default" disabled="disabled" title="Add Selected Users" name="submit_value" value="Add Selected Users" id="user_assign_all" />
    ...
    <input type="hidden" name="sesskey" value="'.sesskey().'"/>
	<input type="hidden" name="options"  value='.$encoded_options.' />
   <label> '.get_string('not_enrolled_users', 'local_groups',$select_to_users_total).$select_all_enrolled_users.'</label>';
...
print_collapsible_region_start(' ', 'filters_form', ' '.' '.'Filters', false, $collapse);
$mform->display();
