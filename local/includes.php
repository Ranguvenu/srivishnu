...
class user_course_details {
	...
	function user_course_completion_progress($courseid, $userid) {
		...
		$course_completion_percent = $completed_activity_count/($total_activity_count-1)*100;
		}
		}else{
...
class has_user_permission{
		...
		$tabs = '<ul class="nav nav-tabs nav-justified">
						<li class="'.$courses_active.'">
							<a data-toggle="tab" href="#plan_courses">
								<span><img src="'.$OUTPUT->pix_url('i/course').'" title="Active History" /></span>
								Course History</a>
						</li>
						<li class="'.$active_active.'">
							<a data-toggle="tab" href="#plan_users">
								<span><img src="'.$OUTPUT->pix_url('i/users').'" title="Assign Users" /></span>
								Active History<span class="badge">'. $total_enroled_users->data .'</span></a>
						</li>
	...
	public function course_history($curr_tab){
		...
		if($curr_tab == 'course'){
			...
			$list[]= date("d-m-Y",$coursehistorylist->enrolleddate);
	...
	$active_courses = ' ';
     ...
     $list[]= date("d-m-Y",$activityhistorylist->enrolledon);
     $list[]= date("d-m-Y",$activityhistorylist->duedate);
     $list[]= date("d-m-Y",$activityhistorylist->coursecompleteddate);
     $list[]= $activityhistorylist->completionpercentage;
     $list[]= gmdate('H:i:s', $activityhistorylist->timespent);
