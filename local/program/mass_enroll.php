...
$curriculum = $DB->get_record('local_program', array('id' => $id));
if (empty($curriculum)) {
...
$PAGE->set_title($curriculum->name . ': ' . $strinscriptions);
$PAGE->set_heading($curriculum->name . ': ' . $strinscriptions);
...
if ($data = $mform->get_data(false)) { // no magic quotes
    ...
    $result = curriculum_mass_enroll($cir, $curriculum, $context, $data);
    ...
    echo $OUTPUT->continue_button(new moodle_url('/local/program/view.php?ccid='.$id.'&prgid='.$curriculum->program)); // Back to course page
    echo $OUTPUT->footer($curriculum);
    die();
}
