...
$PAGE->set_title(get_string('faculty', 'local_program'));
...
$url = new moodle_url($CFG->wwwroot . '/local/program/coursefaculty.php', array('yearid' => $yearid, 'semesterid' => $semesterid, 'courseid' => $courseid));
$PAGE->requires->js_call_amd('local_program/ajaxforms', 'load');

$PAGE->requires->js_call_amd('local_program/program', 'FacultysDatatable',
...
$PAGE->navbar->add($program->fullname, new moodle_url('/local/program/view.php', array('ccid' => $curriculum->id, 'prgid' => $program->id)));
$PAGE->navbar->add(get_string("faculty", 'local_program'));
$PAGE->set_heading(get_string('faculty', 'local_program', $program->fullname));
