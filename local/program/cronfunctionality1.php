...
class cronfunctionality1{
       ...
       // $pluginname = 'program';
        ...
        while($line=$cir->next()){
            ...
            if(count($this->errors) > 0) {
                ...
                // print_r($studentdata);
                // exit;
	            $managefaculty = (new program)->addallstudent($studentdata);
    ...
    function userenrol_validation($excel){
        ...
        $years = $DB->count_records('local_program_cc_years',array('programid'=>$programid));
        $signupyears = $DB->count_records('local_ccuser_year_signups',array('programid'=>$programid,'userid'=>$userid));
        if($years == $signupyears){
            if($DB->record_exists('local_ccuser_year_signups',array('programid'=>$programid,'userid'=>$userid))){
    ...
    } // end of costcenterprogram_validation function
