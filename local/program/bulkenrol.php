...
require_once($CFG->dirroot.'/local/program/cronfunctionality1.php');
...
$yearid = optional_param('yearid', -1, PARAM_INT);
...
$PAGE->set_url('/local/program/bulkenrol.php', array('id'=>$id));

$strinscriptions = get_string('bulkenrol', 'local_program');

$PAGE->set_title($program->fullname . ': ' . $strinscriptions);
$PAGE->set_heading($program->fullname . ': ' . $strinscriptions);

echo $OUTPUT->header();
$mform = new local_courses\form\mass_enroll_form($CFG->wwwroot . '/local/program/bulkenrol.php', array (
    'course' => $program,
    'context' => $context,
    'type' => 'program',
    'ccid'=>$ccid,
...
if ($formdata = $mform->get_data()) { // no magic quotes
    ...
    $result = new local_program\cronfunctionality1();
...
echo html_writer::link(new moodle_url('/local/program/view.php?ccid='.$program->curriculumid.'&prgid='.$program->id.'&type=1'),get_string('back', 'local_courses'),array('id'=>'back_tp_course'));
