...
class online_sessions_integration{
    ...
    public function online_sessions_type($session,$new_session,$type=1,$action){
        ...
        $get_config=get_config('local_curriculum', 'curriculum_onlinesession_type');

        if(!empty($get_config)){
            $instance_type=explode('_',$get_config);
            $visible = $DB->get_field('modules','visible',array('name'=>$instance_type[1]));
            if($visible){
                 $return=$this->curriculum_module_integration($instance_type[1],$session,$new_session,$type);
            }


        }
    }
    /*For curriculum if the Online session is a bigblueButton */
    public function curriculum_module_integration($instance_type,$session,$new_session,$type){
        global $DB,$CFG,$USER;
        ...
        if($instance_type!='webexactivity'){
            $online_module->name=$DB->get_field('local_curriculum','name',array('id'=>$session->curriculumid));
        ...
        $courseid=$DB->get_field('local_curriculum','course',array('id'=>$session->curriculumid));
        if($courseid==0){
           $courseid=1;
        }
        ...
        $online_module->batchid=$session->curriculumid;
        ...
        if($instance_type=='bigbluebuttonbn'){
                ...
                if(!empty($session->curriculumid))
                      $get_trainers=$DB->get_records('local_cc_course_sessions',array('id'=>$session->id));
     ...
     //print_object($online_module);
       $add_moduleinfo=add_moduleinfo($online_module, $courseid, $mform = null);
        ...
        if($add_moduleinfo->id){
            ...
            $DB->update_record('local_cc_course_sessions', $up_session);
        }
