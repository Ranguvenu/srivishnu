...
class assignrole {
	...
	public function rolesassign($users,$roleid,$contextid){
	   ...
	   // print_object($users);
		//foreach($users as $key=>$user){
			role_assign($roleid, $users, $contextid);
			assignuser_parentrole($roleid, $users); 
	   //	}
 	}
 	/**
	 * Function to assign roles to multiple users at once
	 ...
	 */
 	public function get_assignable_roles(\context $context, $rolenamedisplay = ROLENAME_ALIAS, $withusercounts = false, $user = null) {
	    ...
	    if (is_siteadmin($userid)  || has_capability('local/assign_multiple_departments:manage', $context, $userid)) {
	    ...
	    if(!is_siteadmin() && has_capability('local/costcenter:manage_ownorganization',$systemcontext)){
	    	...
	    	if($uhroleid){
	    		$sql .= " AND r.id != $uhroleid";
	    	}
	    }else if(!is_siteadmin() && has_capability('local/costcenter:manage_owndepartments',$systemcontext)){
	        $chroleid = $DB->get_field('role', 'id' ,array('shortname' => 'college_head'));
	    	if($chroleid){
	    		$sql .= " AND r.id != $chroleid";
	    	}
	    }
