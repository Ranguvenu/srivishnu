...
class local_uploadcourse_course {
    ...
    static protected $validfields = array('fullname', 'shortname', 'idnumber', 'category', 'visible', 'startdate', 'enddate','open_points','open_identifiedas','open_costcenterid','open_cost','open_requestcourseid','open_coursecreator','open_coursecompletiondays','open_departmentid','file',
    ...
    public function prepare() {
        ...
        $this->rawdata['categoryname'] = end($categories); 
        ...
        if(!is_siteadmin() && has_capability('local/costcenter:manage_ownorganization',$systemcontext) && !isset($this->rawdata['department']) && isset($this->rawdata['category'])){
            // $categories = $this->get_categories($this->defaults['open_costcenterid']);
            $categories = $this->get_categories($USER->open_costcenterid);
            ...
            $categories = $this->get_categories($departmentid);
            if(!in_array($this->rawdata['category'], $categories)){
                $this->error('canonlycreatecourseincategoryofsameorganisationwithargs', new lang_string('canonlycreatecourseincategoryofsameorganisationwithargs', 'local_courses', $this->rawdata['categoryname']));
                return false;
            }
        }else if(has_capability('local/costcenter:manage_ownorganization',$systemcontext) ||has_capability('local/costcenter:manage_owndepartments', $systemcontext) && isset($this->rawdata['category'])){
            $categories = $this->get_categories($USER->open_departmentid);
            if(!in_array($this->rawdata['category'], $categories)){
                $this->error('canonlycreatecourseincategoryofsameorganisationwithargs', new lang_string('canonlycreatecourseincategoryofsameorganisationwithargs', 'local_courses', $this->rawdata['categoryname']));
                return false;
            }
        ...
        if ($this->rawdata['department']) {
            $departmentid = $DB->get_field('local_costcenter', 'id', array('shortname' => $this->rawdata['department']));
            $this->open_departmentid = $departmentid;
        }
    ...
    public function proceed() {
        ...
        if ($this->do === self::DO_DELETE) {
            ...
            $insertlib->add_enrol_meathod_tocourse($coursedata);
            $this->id = $course->id;
     ...
     */
    protected function get_categories($costcenter){
        global $DB;
