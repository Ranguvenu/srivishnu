...
if((!has_capability('local/costcenter:create', $context)||!has_capability('local/costcenter_course:bulkupload', $context)||!has_capability('local/costcenter_course:manage', $context)||!has_capability('moodle/course:create', $context)||!has_capability('moodle/course:update', $context)) && !is_siteadmin()){
...
if (empty($importid)) {
    ...
    $readcount = $cir->load_csv_content($content, $form1data->encoding, $form1data->delimiter_name);
        ...
        echo $OUTPUT->heading_with_help(get_string('uploadcourses', 'local_courses'), 'uploadcourses', 'tool_uploadcourse');
        $mform1->display();
        echo $OUTPUT->footer();
        die();
    }
