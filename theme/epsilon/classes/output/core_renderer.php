...
class core_renderer extends \core_renderer {
    ...
    public function full_header() {
        ...
        if (empty($PAGE->layout_options['nonavbar'])) {
            $html .= html_writer::start_div('clearfix pull-xs-right', array('id' => 'page-navbar'));
        ...
        $html .= html_writer::div($course_extended_menu, 'pull-xs-right context-header-settings-menu');
    ...
    public function left_navigation_quick_links(){
        ...
        $block_content .= html_writer::start_tag('ul', array('class'=>'pull-left row-fluid user_navigation_ul'));
            ...
            ksort($blocknavs);
            foreach($blocknavs as $blocknav){
                foreach($blocknav  as $key => $value){
                        $data = $value;
                        $block_content .= $data;
                }
            }
            ...
            if($gamificationl_plugin_exist && $gamificationb_plugin_exist){
                $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_gamification_leaderboard', 'class'=>'pull-left user_nav_div notifications row-fluid '));
            ...
            if(!empty($users_plugin_exist)){
                ...
                if(!empty($is_supervisor)){
                    $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_teamdashboard', 'class'=>'pull-left user_nav_div teamdashboard row-fluid '));
                        $supervisor_url = html_writer::link($CFG->wwwroot.'/local/users/team.php',
                                            '<i class="fa fa-users"></i>
                                            <span class="user_navigation_link_text">'.get_string('leftmenu_tm_dashboard', 'theme_epsilon').'</span>',
            ...
            if(is_siteadmin()){
                $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_adminstration', 'class'=>'pull-left user_nav_div adminstration row-fluid '));
    ...
    public function user_nav_header_menu(){
        ...
        $block_content .= html_writer::start_tag('div', array('class'=>'pull-right user_nav_container dropdown'));
            ...
            $block_content .= html_writer::start_tag('ul', array('class'=>'user_nav_header_menu dropdown-menu'));
                //======= Dasboard link ========//  
                $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_dashboard', 'class'=>'pull-left user_nav_div dashboard row-fluid dropdown-item'));
                    $button1 = html_writer::link($CFG->wwwroot, '<i class="fa fa-home" aria-hidden="true"></i><span class="user_navigation_link_text">'.get_string('leftmenu_dashboard', 'theme_epsilon').'</span>', array('class'=>'user_navigation_link'));
                    $block_content .= $button1;
                $block_content .= html_writer::end_tag('li');
            ...
            $block_content .= html_writer::end_tag('li');
                ...
                $gamificationl_plugin_exist = $core_component::get_plugin_directory('local', 'gamification');
                if($gamificationl_plugin_exist && $gamificationb_plugin_exist){
                    $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_gamification_leaderboard', 'class'=>'pull-left user_nav_div notifications row-fluid dropdown-item'));
                    $gamification_url = new moodle_url('/blocks/gamification/dashboard.php');
                    $gamification = html_writer::link($gamification_url, '<i class="fa fa-trophy"></i><span class="user_navigation_link_text">'.get_string('leftmenu_gmleaderboard','theme_epsilon').'</span>',array('class'=>'user_navigation_link'));
                    $block_content .= $gamification;
                    $block_content .= html_writer::end_tag('li');
                ...
                if(!empty($users_plugin_exist)){
                    $is_supervisor = $DB->record_exists('user', array('open_supervisorid' => $USER->id));
                    if(!empty($is_supervisor)){
                        $block_content .= html_writer::start_tag('li', array('id'=> 'id_leftmenu_teamdashboard', 'class'=>'pull-left user_nav_div teamdashboard row-fluid dropdown-item'));
                            $supervisor_url = html_writer::link($CFG->wwwroot.'/local/users/team.php',
                                                '<i class="fa fa-users"></i>
                                                <span class="user_navigation_link_text">'.get_string('leftmenu_tm_dashboard', 'theme_epsilon').'</span>',
                                                array('class'=>'user_navigation_link'));
...
// <mallikarjun> - ODL-645 labels display --- starts
...
// <mallikarjun> - ODL-645 labels display -- ends
            }
        }
    ...
    function get_font_path(){
        ...
        $return = new moodle_url('/theme/epsilon/fonts/poppins.css');
    ...
    */
    public function employee_orgview_buttons() {
        ...
        return $this->render_from_template('theme_epsilon/orgviewbtns', $buttons);
    }
